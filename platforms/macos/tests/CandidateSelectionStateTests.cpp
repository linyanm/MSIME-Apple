...
class Database
{
  public:
    explicit Database(const std::filesystem::path &path)
    {
        ...
        {
            throw std::runtime_error("Failed to create the candidate-selection test dictionary.");
        }
    }
...
void run_tests()
{
    ...
    const std::string leading_candidate = unarmed_session.snapshot().candidates.front().word;
    ...
    const std::string highlighted_candidate = highlighted_session.snapshot().candidates[1].word;
    ...
    const std::string reset_leading_candidate = reset_session.snapshot().candidates.front().word;
    candidate_selection.begin_navigation();
    candidate_selection.update(1, reset_session.snapshot().candidates[1].word);
    candidate_selection.reset();
    require(!candidate_selection.selected_index().has_value(), "Reset retained a stale engine index.");
    const auto reset = candidate_selection.commit(reset_session);
    require(reset.handled && reset.commit == reset_leading_candidate,
    ...
    require(stale.handled && stale.commit == stale_fallback,
    ...
    const auto flushed =
        flush_session.finish(candidate_selection.live_selected_index(flush_session.snapshot()).value_or(0));
    ...
    const std::string flush_default_leading = flush_default_session.snapshot().candidates.front().word;
    candidate_selection.reset();
    ...
    require(paged_session.snapshot().candidates.size() >= 11, "The paging fixture did not return enough candidates.");
    ...
    const auto unavailable_digit = candidate_selection.commit_number(paged_session, '9');
    ...
    const std::string compact_page_candidate = compact_page_session.snapshot().candidates[5].word;
    candidate_selection.begin_navigation();
...
int main()
{
    ...
    const std::filesystem::path data_directory =
        std::filesystem::temp_directory_path() / std::filesystem::u8path("metasequoia-mac-selection-" + suffix);
