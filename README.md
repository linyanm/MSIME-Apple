...
在「系统设置 > 键盘 > 文本输入 > 编辑」中选择水杉输入法，再选择其设置项，即可打开原生的「水杉输入法设置」面板。面板中可以选择全拼、小鹤双拼或 86 五笔，在组词时显示可选的小鹤双拼键位提示，配置唯一四码五笔候选自动上屏，让五笔词库答不上的编码改用同一串字母的拼音候选，预览并切换原生候选窗口的横排与竖排形式，选择 Fluent、微信绿、石墨或杨柳青皮肤（也可从 `~/Library/Application Support/metasequoiaime/skins` 加载与 Windows 相同的 `skin.toml` 外部皮肤），设置每页显示 5、7 或 9 个候选，选择小、标准或大号候选字体，指定 `- / =`、`[ / ]` 或 Page Up / Page Down 作为翻页键，启用全拼纠错与辅助码，为全拼和双拼分别选择 蓝天小雨点、自然码、首右2.0、首右plus 或小鹤 辅助码方案，切换中文标点转换，启用 Option+Shift+H 全角输入，以及控制选中候选是否更新词频学习，并选择一次置顶、折半调频、线性调频或一次置前，以及触发频次与线性步长。启用辅助码后，拼音候选会在括号中显示对应提示，与产品截图中的候选展示一致。初学者键位提示会跟随插入点，高亮最新输入的字母，并在上屏或取消后消失；键盘下方另有一行零声母对照（`a=aa`、`ang=ah`、`eng=eg` 等），这些编码无法从键位本身推出。这些选项按当前用户保存，在没有活动组词时于下一次按键前生效；正在进行的组词会沿用原有设置，直到上屏或取消。后续会继续增加更多输入与候选选项。
